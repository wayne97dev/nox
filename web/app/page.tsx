  ...
  return (
    <div>
      ...
      <section className="relative px-5 sm:px-8 pt-28 pb-24">
        <div className="mx-auto max-w-4xl text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.03] px-4 py-1.5 mb-8 backdrop-blur-md">
            ...
            <span className="text-xs font-normal font-mono tracking-wide text-glow">genesis live on base</span>
          </div>

          <h1 className="text-5xl sm:text-6xl md:text-7xl font-light text-balance mb-6 leading-[1.05] tracking-tight">
            <span className="gradient-text">Private payments</span>
            <br />
            <span className="text-fog">on Base.</span>
          </h1>

          <p className="text-lg md:text-xl text-mist max-w-2xl mx-auto mb-10 text-balance leading-relaxed font-light">
          ...
          {/* Trust strip */}
          <div className="mt-16 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-xs text-mist/70 font-mono">
      ...
      <Section
        ...
        title="Tokenomics"
        subtitle="1,000,000,000 NOX · 18 decimals · immutable supply post-seed"
      >
        ...
        <div className="mx-auto max-w-5xl">
          <Reveal>
            <div className="nox-card ring-glow text-center py-16 px-6">
              ...
              <h3 className="text-3xl font-light text-fog mb-3 tracking-tight">No admin keys. No upgrades.</h3>
              <p className="text-mist mb-8 max-w-xl mx-auto leading-relaxed font-light">
      ...
      <div className="mx-auto max-w-5xl">
        <Reveal>
          <div className="text-center mb-12">
            <div className="text-xs font-normal font-mono uppercase tracking-[0.25em] text-iris/80 mb-3">
              {eyebrow}
            </div>
            <h2 className="text-3xl md:text-4xl font-light text-fog tracking-tight">{title}</h2>
            {subtitle && <p className="mt-3 text-mist font-light">{subtitle}</p>}
          </div>
        </Reveal>
        {children}
      </div>
...
function FeatureCard({
  ...
  return (
    <Card hover className="flex flex-col h-full">
      ...
      <h3 className="text-lg font-normal text-fog mb-2 tracking-tight">{title}</h3>
      <p className="text-sm text-mist leading-relaxed mb-5 flex-1 font-light">{body}</p>
      <Link
        href={href}
        className="inline-flex items-center text-sm font-mono text-iris hover:text-glow transition-colors"
      >
...
function TokenCard({ pct, label, detail }: { pct: string; label: string; detail: string }) {
  return (
    <Card className="text-center py-8 h-full">
      <div className="text-5xl font-mono font-light gradient-text mb-2">{pct}</div>
      <div className="text-fog font-normal mb-1">{label}</div>
  ...
  return (
    <Card hover className="h-full">
      <div className="flex items-start gap-4">
        <div className="font-mono text-sm text-iris/70 shrink-0 mt-0.5">{n}</div>
        <div>
          <h4 className="font-normal text-fog mb-1.5 tracking-tight">{title}</h4>
          <p className="text-sm text-mist leading-relaxed font-light">{body}</p>
        </div>
      </div>
    </Card>
  );
